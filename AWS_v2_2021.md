...
- After you connect to your AWS instance, you need to run the following command to initialize your ~/CourseData storage otherwise your labs will be extremely slow. The command will run in background and there will be no output. You want to run this command as soon as possible because it may take hours depends on the size of data stes and how busy AWS is.

```
~/init.sh &
```

- To chane your password (user ubunbtu), you can do
```
sudo passwd ubuntu
```
